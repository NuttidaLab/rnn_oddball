...
# 1/29/2024 -- copy of main.py to make the training of go-nogo task sequential modeled after
# the auditory oddball task used for pixel data
...
from model import generate_input_stim_go_nogo2
...
parser.add_argument("--prob", required=False,
        type=restricted_float, default = 0.50,
        help="Probability for GO trials")
...
if args.task.lower() == 'go-nogo' or args.task.lower() == 'go-nogo2':
    # GO-NoGo task
    settings = {
            'T': 200, # trial duration (in steps)
            'stim_on': 50, # input stim onset (in steps)
            'stim_dur': 25, # input stim duration (in steps)
            'prob': args.prob, # probability for GO signal (0.70 = 70% chance GO)
...
# Learning stages (0: 70/30, 1: 50/50, 2: 30/70)
stage = 0
...
if args.mode.lower() == 'train':
    with tf.Session() as sess:
        ...
        if args.task.lower() == 'go-nogo':
            ...
            # Go-NoGo task
            u, label = generate_input_stim_go_nogo2(settings)
            target = generate_target_continuous_go_nogo(settings, label)
            x0, r0, w0, w_in0, taus_gaus0 = \
                    sess.run([x, r, w, w_in, taus], feed_dict={input_node: u, z: target})
        ...
        for tr in range(args.n_trials):
            ...
            if stage == 0:
                settings['prob'] = 1 - args.prob 
            elif stage == 1:
                settings['prob'] = 0.50
            elif stage == 2:
                settings['prob'] = args.prob
            ...
            print("Stage " + str(stage) + " Trial " + str(tr) + ': ' + str(label))
            ...
            if args.task.lower() == 'go-nogo':
                ...
                if (tr-1)%training_params['eval_freq'] == 0:
                    ...
                    if eval_loss_mean < training_params['loss_threshold'] and eval_perf_mean > 0.85:
                        ...
                        if len(settings['taus']) > 1:
                            fname = 'Stage_{}_Prob_{}_Task_{}_N_{}_Taus_{}_{}_Act_{}_{}.mat'.format(stage, settings['prob'], args.task.lower(), N, settings['taus'][0], 
                                    settings['taus'][1], training_params['activation'], fname_time)
                        elif len(settings['taus']) == 1:
                            fname = 'Stage_{}_Prob_{}_Task_{}_N_{}_Tau_{}_Act_{}_{}.mat'.format(stage, settings['prob'], args.task.lower(), N, settings['taus'][0], 
                ...
                if (tr-1)%training_params['eval_freq'] == 0:
                    ...
                    eval_labels = np.zeros((training_params['eval_tr'], ))
                    for ii in range(eval_perf.shape[-1]):
                        eval_u, eval_label = generate_input_stim_go_nogo2(settings)
                        eval_target = generate_target_continuous_go_nogo(settings, eval_label)
                        eval_o, eval_l = sess.run([o, loss], feed_dict = \
                                {input_node: eval_u, z: eval_target})
                        eval_losses[0, ii] = eval_l
                    ...
                    if (tr-1)%(5*training_params['eval_freq']) == 0:
                        # Save the trained params in a .mat file
                        var = {}
                        var['x0'] = x0
                        var['r0'] = r0
                        ...
                        var['activation'] = training_params['activation']
                        if len(settings['taus']) > 1:
                            fname = 'Stage_{}_Tr_{}_Prob_{}_Task_{}_N_{}_Taus_{}_{}_Act_{}_{}.mat'.format(stage, tr, settings['prob'], args.task.lower(), N, settings['taus'][0], 
                                    settings['taus'][1], training_params['activation'], fname_time)
                        elif len(settings['taus']) == 1:
                            fname = 'Stage_{}_Tr_{}_Prob_{}_Task_{}_N_{}_Tau_{}_Act_{}_{}.mat'.format(stage, tr, settings['prob'], args.task.lower(), N, settings['taus'][0], 
                                    training_params['activation'], fname_time)
                        scipy.io.savemat(os.path.join(out_dir, fname), var)

                    if eval_loss_mean < training_params['loss_threshold'] and eval_perf_mean > 0.95:
                        if len(settings['taus']) > 1:
                            fname = 'END_Stage_{}_Prob_{}_Task_{}_N_{}_Taus_{}_{}_Act_{}_{}.mat'.format(stage, settings['prob'], args.task.lower(), N, settings['taus'][0], 
                                    settings['taus'][1], training_params['activation'], fname_time)
                        elif len(settings['taus']) == 1:
                            fname = 'END_Stage_{}_Prob_{}_Task_{}_N_{}_Tau_{}_Act_{}_{}.mat'.format(stage, settings['prob'], args.task.lower(), N, settings['taus'][0], 
                                    training_params['activation'], fname_time)
                        scipy.io.savemat(os.path.join(out_dir, fname), var)
                        if stage < 3:
                            stage += 1
