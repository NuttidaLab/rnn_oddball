        ...
        for tr in range(args.n_trials):
            ...
            if args.task.lower() == 'go-nogo':
                ...
                if (tr-1)%training_params['eval_freq'] == 0:
                    ...
                    var['activation'] = training_params['activation']
                    if len(settings['taus']) > 1:
                        fname = 'Stage_{}_Tr_{}_Prob_{}_Task_{}_N_{}_Taus_{}_{}_Act_{}_{}_2stages.mat'.format(stage, tr, settings['prob'], args.task.lower(), N, settings['taus'][0], 
                                settings['taus'][1], training_params['activation'], fname_time)
                    elif len(settings['taus']) == 1:
                        fname = 'Stage_{}_Tr_{}_Prob_{}_Task_{}_N_{}_Tau_{}_Act_{}_{}_2stages.mat'.format(stage, tr, settings['prob'], args.task.lower(), N, settings['taus'][0], 
                                training_params['activation'], fname_time)
                    scipy.io.savemat(os.path.join(out_dir, fname), var)

                    if eval_loss_mean < training_params['loss_threshold'] and eval_perf_mean > 0.95:
                        if len(settings['taus']) > 1:
                            fname = 'END_Stage_{}_Prob_{}_Task_{}_N_{}_Taus_{}_{}_Act_{}_{}_2stages.mat'.format(stage, settings['prob'], args.task.lower(), N, settings['taus'][0], 
                                    settings['taus'][1], training_params['activation'], fname_time)
                        elif len(settings['taus']) == 1:
                            fname = 'END_Stage_{}_Prob_{}_Task_{}_N_{}_Tau_{}_Act_{}_{}_2stages.mat'.format(stage, settings['prob'], args.task.lower(), N, settings['taus'][0], 
                                    training_params['activation'], fname_time)
                        scipy.io.savemat(os.path.join(out_dir, fname), var)
                        if stage < 2:
                            stage += 1
